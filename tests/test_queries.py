...
def test_filter_by_status_group(seeded):
    ...
    _, total = search_meetings(
        seeded, MeetingFilters(statuses=["in_meeting", "upcoming"])
    )
    assert total == 0
...
def test_filter_by_user_status(seeded):
    ...
    _, total = search_meetings(
        seeded, MeetingFilters(statuses=["ready", "failed", "no_recording"])
    )
    assert total == 3
...
def test_status_sort_ranks_lifecycle_first(seeded):
    ...
    assert [r.user_status for r in rows] == [
        "to_process",
        "ready",
        "failed",
        "no_recording",
...
def test_sort_orders(seeded):
    ...
    assert [r.title for r in rows] == sorted(
        (r.title for r in rows), key=str.lower, reverse=True
    )
